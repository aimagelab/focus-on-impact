...
from intrinsic_baselines.common.curiosity_models_hierarchical import CuriosityModels
from intrinsic_baselines.common.models import ForwardModel, InverseModel
from intrinsic_baselines.common.rollout_storage_hierarchical import RolloutStorageHierarchical
from intrinsic_baselines.common.utils import append_observations
...
@baseline_registry.register_trainer(name="ppo_curiosity_hierarchical")
class PPOTrainerCuriosityHierarchical(BaseRLTrainer):
    ...
    def __init__(self, config=None):
        ...
        self.curiosity_obs = None
        self.curiosity_hidden_states = None
        self.episode_curiosity_rewards = None
    ...
    def _setup_actor_critic_agent(self, ppo_cfg: Config, ans_cfg: Config) -> None:
        ...
        self.curiosity_models = CuriosityModels(
            ...
            fwd_model=ForwardModel(self.envs.action_spaces[0].n,
                                   state_size=ans_cfg.CURIOSITY.curiosity_hidden_size,
                                   hidden_size=ans_cfg.CURIOSITY.curiosity_hidden_size // 2),
            inv_model=InverseModel(self.envs.action_spaces[0].n,
                                   state_size=ans_cfg.CURIOSITY.curiosity_hidden_size,
                                   hidden_size=ans_cfg.CURIOSITY.curiosity_hidden_size // 2),
            curiosity_hidden_size=ans_cfg.CURIOSITY.curiosity_hidden_size,
            device=self.device,
        ...
        self.curiosity_forward_model = self.curiosity_models.fwd_model
        self.curiosity_inverse_model = self.curiosity_models.inv_model
        self.curiosity_encoder = self.curiosity_models.obs_encoder
        ...
        self.curiosity_forward_model.to(self.device)
        self.curiosity_inverse_model.to(self.device)
        self.curiosity_encoder.to(self.device)
        self.depth_projection_net.to(self.device)

        self.ans_net.to_ddp()
        self.curiosity_models.to_ddp(device_ids=ans_cfg.gpu_ids, output_device=ans_cfg.gpu_ids[0])
        # ============================== Create agents ================================
        # Mapper agent
        ...
        self.episode_curiosity_rewards = torch.zeros(self.envs.num_envs, 1)
    ...
    def save_checkpoint(
        ...
        checkpoint = {
            ...
            "forward_model": self.curiosity_forward_model.state_dict(),
            "inverse_model": self.curiosity_inverse_model.state_dict(),
            "curiosity_encoder": self.curiosity_encoder.state_dict(),
            "config": self.config,
        }
    ...
    def resume_checkpoint(self, path=None):
        ...
        if path is None:
            if len(checkpoints) == 0:
                ...
                self.curiosity_forward_model.load_state_dict(ckpt_dict["forward_model"])
                self.curiosity_inverse_model.load_state_dict(ckpt_dict["inverse_model"])
                self.curiosity_encoder.load_state_dict(ckpt_dict["curiosity_encoder"])
    ...
    def _collect_rollout_step(
        ...
        for local_step in range(NUM_LOCAL_STEPS):
            ...
            # print(f'===> Local time: {local_step}, episode time: {episode_step_count[0].item()}')
            # ---------------------------- sample actions -----------------------------
            ...
            if local_step == 0:
                ...
                if hasattr(self.config.RL.PPO,
                           'global_reward_type') and 'curiosity' in self.config.RL.PPO.global_reward_type:
                    global_reward_metric = self.episode_curiosity_rewards
                else:
            ...
            if hasattr(self.config.RL.PPO,
                       'global_reward_type') and 'cur' in self.config.RL.PPO.global_reward_type:
                new_curiosity_obs = append_observations(self.config, self.curiosity_obs, observations,
                                                        self.device)
                # Update reward for previous global_policy action
                self.curiosity_encoder.eval()

                with torch.no_grad():
                    features, self.curiosity_hidden_states = self.curiosity_encoder(
                        self._dict_to_device(self.curiosity_obs, device),
                        self.curiosity_hidden_states,
                        None, masks.to(device),
                        use_rnn=ans_cfg.CURIOSITY.use_curiosity_rnn)
                    new_features, _ = self.curiosity_encoder(self._dict_to_device(new_curiosity_obs, device),
                                                             self.curiosity_hidden_states,
                                                             None, new_masks.to(device),
                                                             use_rnn=ans_cfg.CURIOSITY.use_curiosity_rnn)

                self.curiosity_encoder.train()

                curiosity_rewards = self._compute_curiosity_reward(features, new_features, actions)
                self.episode_curiosity_rewards = self.episode_curiosity_rewards.to(
                    device) + curiosity_rewards.clone().detach()

                if self.curiosity_hidden_states is None:
                    self.curiosity_hidden_states = torch.zeros_like(curiosity_rollouts.recurrent_hidden_states[0])
                self.curiosity_obs = new_curiosity_obs
                del new_curiosity_obs

                curiosity_rollouts.insert(
                    self.curiosity_obs,
                    self.curiosity_hidden_states,
                    actions,
                    curiosity_rewards,
    ...
    def _supplementary_rollout_update(
        ...
        if update_option == "global":
            ...
            if hasattr(self.config.RL.PPO,
                       'global_reward_type') and self.config.RL.PPO.global_reward_type == 'curiosity':
                global_reward_metric = self.episode_curiosity_rewards
            else:
                global_reward_metric = self._compute_mapbased_global_metric(
                    ground_truth_states, mapper_outputs, state_estimates
    ...
    def _update_curiosity_models(self, curiosity_rollouts):

        self.curiosity_models.train()
        ...
        (
            fwd_loss,
            inv_loss
        ) = self.curiosity_models.update(curiosity_rollouts, self.config.RL.ANS)

        curiosity_rollouts.after_update()
    ...
    def _create_curiosity_rollouts(self, ppo_cfg, ans_cfg):
        ...
        if 'DEPTH_SENSOR' in self.config['SENSORS']:
            rollout_obs_spaces.spaces['depth'] = Box(low=0.0, high=1.0,
                                                     ...
                                                     dtype='float32')
        curiosity_rollouts = RolloutStorageHierarchical(
            ppo_cfg.num_local_steps,
            self.envs.num_envs,
            rollout_obs_spaces,
            self.envs.action_spaces[0],
            ans_cfg.CURIOSITY.curiosity_hidden_size,
            num_recurrent_layers=self.curiosity_encoder.num_recurrent_layers
        )

        curiosity_obs = dict()
        if 'rgb' in curiosity_rollouts.observations.keys():
            curiosity_obs['rgb'] = torch.zeros_like(curiosity_rollouts.observations['rgb'][0])
        if 'depth' in curiosity_rollouts.observations.keys():
            curiosity_obs['depth'] = torch.zeros_like(curiosity_rollouts.observations['depth'][0])

        return curiosity_obs, curiosity_rollouts
    ...
    def _compute_curiosity_reward(self, features, new_features, actions):
        action_onehot = torch.zeros(self.config.NUM_PROCESSES,
                                    self.envs.action_spaces[0].n,
                                    device=self.device)
        action_onehot.scatter_(1, actions.view(-1, 1).long(), 1)

        prev_features = features
        self.curiosity_forward_model.eval()
        with torch.no_grad():
            pred_features = self.curiosity_forward_model(prev_features, action_onehot)
        self.curiosity_forward_model.train()

        return 0.5 * torch.mean(
            F.mse_loss(pred_features, new_features, reduction='none'),
            dim=1).view(-1, 1)
    ...
    def train(self) -> None:
        ...
        self.curiosity_obs, curiosity_rollouts = self._create_curiosity_rollouts(ppo_cfg, ans_cfg)

        global_rollouts.to(self.device)
        local_rollouts.to(self.device)
        curiosity_rollouts.to(self.device)
        if ans_cfg.CURIOSITY.use_curiosity_rnn:
            self.curiosity_hidden_states = torch.zeros_like(curiosity_rollouts.recurrent_hidden_states[0])
        ...
        # Curiosity models statistics
        statistics_dict["curiosity_models"] = defaultdict(
            lambda: deque(maxlen=ppo_cfg.loss_stats_window_size)
        )
        ...
        total_memory_size = 0
        print("================== Curiosity rollouts ====================")
        for k, v in curiosity_rollouts.observations.items():
            mem = v.element_size() * v.nelement() * 1e-9
            print(f"key: {k:<40s}, memory: {mem:>10.4f} GB")
            total_memory_size += mem
        print(f"Total memory: {total_memory_size:>10.4f} GB")
        ...
        self.curiosity_obs = append_observations(self.config, self.curiosity_obs, observations, self.device)
        # Insert first observation in curiosity rollouts
        for sensor in self.curiosity_obs:
            curiosity_rollouts.observations[sensor][0].copy_(
                self.curiosity_obs[sensor]
            )
        # Set starting action
        curiosity_rollouts.prev_actions[0] = torch.ones_like(
            curiosity_rollouts.prev_actions[0]) * (len(
        ...
        for update in tqdm(range(num_updates_start, NUM_GLOBAL_UPDATES)):
            self.update = update
            for global_step in tqdm(range(NUM_GLOBAL_STEPS)):
                ...
                if UPDATE_CURIOSITY_FLAG:

                    # Sanity check
                    assert curiosity_rollouts.step == local_rollouts.num_steps

                    pth_time += delta_pth_time
                    (
                        delta_pth_time,
                        update_metrics_curiosity
                    ) = self._update_curiosity_models(curiosity_rollouts)

                    for k, v in update_metrics_curiosity.items():
                        statistics_dict["curiosity_models"][k].append(v)
            ...
            if episode_step_count[0].item() == self.config.T_EXP:
                ...
                self.episode_curiosity_rewards *= masks.cuda()
                for k in self.curiosity_obs.keys():
                    self.curiosity_obs[k].fill_(0)
                ...
                # Handle curiosity observations
                self.curiosity_obs = append_observations(self.config, self.curiosity_obs, observations,
                                                         self.device)
                # Insert first observation in curiosity rollouts
                for sensor in self.curiosity_obs:
                    curiosity_rollouts.observations[sensor][0].copy_(
                ...
                batch = self._prepare_batch(observations)
                prev_batch = batch
                # Reset episode global_step counter
    ...
    def _eval_checkpoint(
        ...
        for ep in tqdm(range(number_of_eval_episodes)):
            ...
            if ep < len(increm_per_episode_statistics):
            ...
            for i, episode in enumerate(current_episodes):
                scene_names.append(episode.scene_id.split('/')[-1])
                map_variants.append(episode.manipulated_map_path)
            batch = self._prepare_batch(observations)
            prev_batch = batch
            state_estimates = {
                "pose_estimates": torch.zeros(self.envs.num_envs, 3).to(self.device),
            ...
            for ep_step in tqdm(range(self.config.T_EXP)):
                ...
                if ep_step == 0:
                    ...
                    # imageio.imwrite('env_layout.jpg', environment_layout[0, 0].cpu().numpy())
            ...
            if self.config.SAVE_STATISTICS_FLAG:
                ...
                if not self.config.QUALITATIVE:

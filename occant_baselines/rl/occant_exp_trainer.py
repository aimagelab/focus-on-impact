...
class OccAntExpTrainer(BaseRLTrainer):
    ...
    def _compute_global_metric(self, ground_truth_states, mapper_outputs, state_estimates=None):
    ...
    def _collect_rollout_step(
        ...
        for t in range(NUM_LOCAL_STEPS):
            ...
            for i, obs in enumerate(observations):
                if 'scene_name' in observations[i]:
    ...
    def train(self) -> None:
        ...
        map_variants = []
        for i, obs in enumerate(observations):
            if 'scene_name' in obs.keys():
                scene_names.append(obs['scene_name'])
                map_variants.append(current_episodes[i].manipulated_map_path)
                del observations[i]['scene_name']
        ...
        for update in tqdm(range(num_updates_start, NUM_GLOBAL_UPDATES)):
            ...
            if update % self.config.CHECKPOINT_INTERVAL == 0:
                if self._is_master_process():
                    ...
                    if os.path.isfile(
                            os.path.join(self.config.CHECKPOINT_FOLDER, f"ckpt.{count_checkpoints - 1}.pth")) and (
                            count_checkpoints - 1) % 10 != 0:
                        os.remove(os.path.join(self.config.CHECKPOINT_FOLDER, f"ckpt.{count_checkpoints - 1}.pth"))
                    count_checkpoints += 1
